...
`paperforge` 中的论文转专利模块适合作为技术写作起点，但若直接用于完整
中国发明专利初稿，仍有以下不足：

1. 主要输出是摘要、背景、发明内容、实施方式和少量权利要求种子，不是完整申请包；
2. 没有把每项权利要求特征逐项映射到论文原文、公式、附图或代码证据；
3. 没有规定核心公式必须进入说明书并生成为 Word 原生可编辑公式；
4. 没有生成与独立权利要求逐步一致的摘要附图和说明书附图；
5. 没有对权利要求引用、术语一致性、公式覆盖、附图对齐和交付门槛进行自动检查；
6. 对扫描 PDF、论文与已有专利对照、发明人待确认事项和公开时间风险覆盖不足。

当前版本在保留其“先抽取技术问题、技术方案和技术效果”这一合理起点的
基础上，增加了完整来源地图、证据台账、阶段门、结构化草稿、公式与附图
流水线、自动验证和分立 DOCX 交付。
